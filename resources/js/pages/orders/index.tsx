...
export default function OrdersIndex({ orders }: OrdersPageProps) {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(orders.per_page.toString());
  ...
  const handlePageChange = (url: string) => {
    ...
    if (paymentStatusFilter !== 'all') {
      searchParams.payment_status = paymentStatusFilter;
    }
  ...
  const handleItemsPerPageChange = (value: string) => {
    ...
    router.get('/orders', searchParams, { preserveState: true });
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {orders.data.filter(order => order.status === 'Processing').length}
                </div>
